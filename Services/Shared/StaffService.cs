...
public class StaffService : IStaffService
{
    ...
    public async Task<Staff> CreateStaffFromAzureAsync(ClaimsPrincipal principal)
    {
        var staff = new Staff
        {
    ...
    public async Task<Staff> UpdateStaffFromAzureAsync(Staff staff, ClaimsPrincipal principal)
    {
        // 更新基本資訊
        ...
        staff.LastSyncAt = DateTime.UtcNow;
    ...
    /// 嘗試取得Azure AD頭像URL
    /// </summary>
    private void TryGetPhotoUrlAsync(Staff staff, ClaimsPrincipal principal)
    {
        try
        {
            // 方法1：從Claims取得（如果有）
