...
public class BlobService : IBlobService
{
        ...
        bool isTemp = false)
    {
        ...
        var blobStorage = new BlobStorage
        {
            ...
            UploadedAt = DateTimeExtensions.TaiwanTime,
            IsTemp = isTemp,
            ExpiresAt = isTemp ? DateTimeExtensions.TaiwanTime.AddDays(7) : null,
    ...
    public async Task<string> GetFileUrlAsync(int blobId)
    {
        ...
        // 總是產生 SAS URL，不使用資料庫中儲存的 URL
        // 因為資料庫中儲存的是普通 URL，在 Storage Account 不允許公開訪問時會失敗
        var containerClient = _blobServiceClient.GetBlobContainerClient(blobStorage.ContainerName);
        var blobClient = containerClient.GetBlobClient(blobStorage.BlobName);
    ...
    public async Task<bool> DeleteFileAsync(int blobId)
    {
        ...
        try
        {
            ...
            blobStorage.DeletedAt = DateTimeExtensions.TaiwanTime;
            blobStorage.UpdatedAt = DateTimeExtensions.TaiwanTime;
            await _context.SaveChangesAsync();
